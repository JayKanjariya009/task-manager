...
function AdminTasks() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const users = res.data
        ...
        .map(task => ({ id: task.userId._id, username: task.userId.username }));
      
      const uniqueUsersMap = {};
      users.forEach(user => {
        uniqueUsersMap[user.id] = user.username;
      });
      
      const uniqueUsersList = Object.keys(uniqueUsersMap).map(id => ({
        id,
        username: uniqueUsersMap[id]
      }));
  ...
  const getTaskStats = () => {
    ...
    const usersWithTasks = new Set(
      tasks
        .filter(task => task.userId && task.userId._id)
        .map(task => task.userId._id)
    ).size;
    
    return { total, completed, inProgress, pending, overdue, usersWithTasks };
  };
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    const userMatch = filterUser === "all" || 
      (task.userId && task.userId._id === filterUser);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          ...
          <div className="bg-white p-4 rounded-lg shadow-md border-l-4 border-blue-500">
            ...
            <p className="text-2xl font-bold text-blue-700">{stats.usersWithTasks}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <h2 className="text-2xl font-semibold text-indigo-800">All Tasks</h2>
            ...
            {uniqueUsers.length > 0 && (
              <select 
                className="bg-white border border-gray-200 rounded-lg shadow-sm px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                value={filterUser}
